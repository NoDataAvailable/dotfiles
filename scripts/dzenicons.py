...
# Battery/Power
bat_icons   = ["ac_01","bat_full_02","bat_full_02","bat_low_02","bat_low_02","bat_empty_02"]
bat_colours = [cyan,green,yellow,yellow,red,red]

ac_status   = "/sys/class/power_supply/AC0/online"
bat_dir     = "/sys/class/power_supply/BAT0/"
bat_full    = bat_dir + "energy_full"
bat_curr    = bat_dir + "energy_now"
bat_status  = 0
...
gpu_temp = "/sys/class/hwmon/hwmon0/device/temp1_input"
...
#        gpu_state = (gpu_temperature - 40) / 15
 #   output += '  |' + new_icon(gpu_colours[gpu_state], gpu_icon, gpu_temperature, 25) 
  #  slave +=  ' [' + str(gpu_temperature) + " C]"
    ...
    # Power
    plugged = iValue(ac_status)
    bat_percent = 0#int(fValue(bat_curr)/fValue(bat_full)*100)
    if plugged:
        bat_status = 0
    else:
        bat_status = 5 - int((bat_percent-1)/20)
    if bat_percent < 4:
        bat_percent += 4

    if not bat_status:
        charge_state = "net_up_02"
    else:
        charge_state = "net_down_02"
    output += '  |' + new_icon(bat_colours[bat_status], bat_icons[bat_status], bat_percent-3, 40)
    slave  += '  |  ' +  ' [' + "%3d" % (bat_percent) + '%]' +'  '#^i(' + icon(charge_state) + ')'

    if net_state == 1 or net_state == 4:
        count = 0
    else:
        count += 1
